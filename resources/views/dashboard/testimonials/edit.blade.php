            ...
            $('#mainphoto').dropzone({
                ...
                init: function () {
                    ...
                    @if(!empty($testimonial->image))
                        ...
                        this.emit('thumbnail', mock, '{{ $testimonial->thumb_image }}');
                ...
                <form method="post" action="{{ route('testimonials.update', $testimonial->id) }}" enctype="multipart/form-data">
                    ...
                    @foreach(config('locales.languages') as $key => $value)
                        <div class="form-group">
                            ...
                            <input id="name[{{ $key }}]" name="name[{{ $key }}]" type="text" class="form-control" placeholder="{{ trans('admin.'. $key .'_name') }}" value="{{ old('name.' . $key, $testimonial->getTranslation('name', $key)) }}">
                        </div>
                    @endforeach
                    ...
                    @foreach(config('locales.languages') as $key => $value)
                        <div class="form-group">
                            ...
                            <input id="title[{{ $key }}]" name="title[{{ $key }}]" type="text" class="form-control" placeholder="{{ trans('admin.'. $key .'_title') }}" value="{{ old('title.' . $key, $testimonial->getTranslation('title', $key)) }}">
                        </div>
                    @endforeach
                    ...
                    @foreach(config('locales.languages') as $key => $value)
                        <div class="form-group">
                            ...
                            <textarea id="desc[{{ $key }}]" name="description[{{ $key }}]" rows="4" class="form-control" placeholder="{{ trans('admin.post') }} / {{ trans('admin.'. $key .'_description') }}...">{{ old('description.' . $key, $testimonial->getTranslation('description', $key)) }}</textarea>
                        </div>
                    @endforeach
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="stars" id="stars" class="form-control select2">
                                    @for($i = 1;$i < 6;$i++)
                                        <option value="{{ $i }}" {{ $testimonial->stars == $i ? 'selected' : '' }}> {{ $i }} {{ __('home.star') }}</option>
                                    @endfor
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="status" id="status" class="form-control select2">
                                    @for($i = 0;$i < 2;$i++)
                                        <option value="{{ $i }}" {{ $testimonial->status == $i ? 'selected' : '' }}> {{ $i == 0 ? __('admin.hidden') : __('admin.visible') }}</option>
                                    @endfor
                                </select>
                            </div>
                        </div>
                    ...
                    @foreach(config('locales.languages') as $key => $value)
                        <div class="form-group">
                            ...
                            <input id="meta_tag[{{ $key }}]" name="meta_tag[{{ $key }}]" type="text" class="form-control" placeholder="{{ trans('admin.'. $key .'_meta_tag') }}" value="{{ old('meta_tag.' . $key, $testimonial->getTranslation('meta_tag', $key)) }}">
                        </div>
                    @endforeach
