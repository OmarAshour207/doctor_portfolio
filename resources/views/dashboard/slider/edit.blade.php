            ...
            $('#mainphoto').dropzone({
                ...
                init: function () {
                    ...
                    @if(!empty($slider->image))
                        ...
                        this.emit('thumbnail', mock, '{{ $slider->thumb_image }}');
                ...
                <form method="post" action="{{ route('slider.update', $slider->id) }}" enctype="multipart/form-data">
                    ...
                    @foreach(config('locales.languages') as $key => $value)
                        <div class="form-group">
                            <label for="title[{{ $key }}]"> {{ trans('admin.slider') }} / {{ trans('admin.'. $key .'_title') }}</label>
                            <input id="title[{{ $key }}]" name="title[{{ $key }}]" type="text" class="form-control" placeholder="{{ trans('admin.'. $key .'_title') }}" value="{{ old('title.' . $key, $slider->getTranslation('title', $key)) }}">
                        </div>
                    @endforeach

                    @foreach(config('locales.languages') as $key => $value)
                        <div class="form-group">
                            <label for="desc[{{ $key }}]"> {{ trans('admin.slider') }} / {{ trans('admin.'. $key .'_description') }}</label>
                            <textarea id="desc[{{ $key }}]" name="description[{{ $key }}]" rows="4" class="form-control" placeholder="{{ trans('admin.post') }} / {{ trans('admin.'. $key .'_description') }}...">{{ old('body.' . $key, $slider->getTranslation('description', $key)) }}</textarea>
                        </div>
                    @endforeach
