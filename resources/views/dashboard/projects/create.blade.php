                ...
                <form method="post" action="{{ route('projects.store') }}" enctype="multipart/form-data">
                    ...
                    @foreach(config('locales.languages') as $key => $value)
                        <div class="form-group">
                            ...
                            <textarea id="desc[{{ $key }}]" name="description[{{ $key }}]" rows="4" class="form-control" placeholder="{{ trans('admin.projects') }} / {{ trans('admin.'. $key .'_description') }}...">{{ old('_description.' . $key) }}</textarea>
                        </div>
                    @endforeach

                    @foreach(config('locales.languages') as $key => $value)
                        <div class="form-group">
                            <label for="meta_tag[{{ $key }}]"> {{ trans('admin.projects') }} / {{ trans('admin.'. $key .'_meta_tag') }}</label>
                            <input id="meta_tag[{{ $key }}]" name="meta_tag[{{ $key }}]" type="text" class="form-control" placeholder="{{ trans('admin.'. $key .'_meta_tag') }}" value="{{ old('_meta_tag.' . $key) }}">
                        </div>
                    @endforeach
